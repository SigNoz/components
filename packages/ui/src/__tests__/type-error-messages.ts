/**
 * Shared harness for the `*.types.messages.test.ts` suites.
 *
 ...
 * unreadable one.
 *
 * This closes that gap. It compiles a `*.types.test-d.tsx` fixture with the `@ts-expect-error`
 * directives blanked out, then snapshots the diagnostic each case actually produces, keyed by
...
/** `packages/ui/tsconfig.json`, resolved from this file so callers never spell out `../../..`. */
const tsconfigPath = resolve(dirname(fileURLToPath(import.meta.url)), '../../tsconfig.json');
...
interface ExpectedError {
	/** 1-based line the directive suppresses, i.e. the line holding the opening JSX tag. */
	line: number;
	label: string;
}
 ...
 */
function readFixture(fixturePath: string): { source: string; expected: ExpectedError[] } {
 ...
 */
function compileFixture(fixturePath: string, source: string): ts.Diagnostic[] {
...
/** Groups the fixture's diagnostics by the 1-based line they were reported on. */
function diagnosticsByLine(fixturePath: string, source: string): Map<number, string[]> {
	const byLine = new Map<number, string[]>();

	for (const diagnostic of compileFixture(fixturePath, source)) {
...
interface TypeErrorMessagesOptions {
	/** Component name, used for the suite title only. */
	component: string;
	/** `import.meta.url` of the calling test file, which the fixture is resolved against. */
	testFile: string;
	/** Fixture filename, e.g. `button.types.test-d.tsx`. */
	fixture: string;
}

/**
 * Declares the three assertions that guard a component's type error messages. The snapshot lands
 * next to the calling test file, so each component keeps its own `__snapshots__` entry.
 */
export function describeTypeErrorMessages({
	component,
	testFile,
	fixture,
}: TypeErrorMessagesOptions): void {
	const fixturePath = resolve(dirname(fileURLToPath(testFile)), fixture);
	const { source, expected } = readFixture(fixturePath);
	const byLine = diagnosticsByLine(fixturePath, source);

	describe(`${component} type error messages`, () => {
