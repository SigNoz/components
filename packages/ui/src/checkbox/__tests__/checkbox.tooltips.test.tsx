...
describe('Checkbox disabledTooltip', () => {
	it('shows the reason on hover', async () => {
		const user = userEvent.setup();
		render(
			<Checkbox color="primary" testId="checkbox" disabled disabledTooltip={DISABLED_REASON}>
				Accept the terms
			</Checkbox>,
		);

		await user.hover(screen.getByTestId('checkbox'));
	...
	it('shows the reason from the label too: the whole row is the trigger', async () => {
		const user = userEvent.setup();
		render(
			<Checkbox color="primary" disabled disabledTooltip={DISABLED_REASON}>
				Accept the terms
			</Checkbox>,
		);

		await user.hover(screen.getByText('Accept the terms'));

		expect(await screen.findByRole('tooltip')).toHaveTextContent(DISABLED_REASON);
	});

	it('shows the reason on a bare checkbox as well', async () => {
		const user = userEvent.setup();
		render(
			<Checkbox
				color="primary"
	...
	it('says nothing while the checkbox is usable', async () => {
		const user = userEvent.setup();
		render(
			<Checkbox
				color="primary"
...
describe('Checkbox readOnlyTooltip', () => {
	it('shows the reason on hover', async () => {
		const user = userEvent.setup();
		render(
			<Checkbox color="primary" testId="checkbox" readOnly readOnlyTooltip={READ_ONLY_REASON}>
				Accept the terms
			</Checkbox>,
		);
...
describe('Checkbox tooltip stacking', () => {
