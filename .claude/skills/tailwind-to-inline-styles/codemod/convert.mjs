...
const STATIC = {
	...
	'border-r': [['borderRight', "'1px solid var(--border)'"]],
...
function mapClass(cls, flags) {
	...
	if (
		/^(dark|hover|focus|focus-visible|active|disabled|group-hover|peer|sm|md|lg|xl|2xl|first|last|odd|even|aria-|data-):/.test(
			cls,
		)
	) {
		flags.add(cls);
	...
	if ((m = cls.match(/^text-(xs|sm|base|lg|xl|2xl|3xl|4xl|5xl|6xl)$/)))
		return [['fontSize', num(FONT_SIZE[m[1]])]];
	// leading
	...
	if ((m = cls.match(/^grid-cols-(\d+)$/)))
		return [['gridTemplateColumns', num(`repeat(${m[1]}, minmax(0, 1fr))`)]];
	if ((m = cls.match(/^grid-cols-\[(.+)\]$/))) return [['gridTemplateColumns', num(m[1])]];
	if ((m = cls.match(/^grid-rows-(\d+)$/)))
		return [['gridTemplateRows', num(`repeat(${m[1]}, minmax(0, 1fr))`)]];
	if ((m = cls.match(/^col-span-(\d+)$/)))
		return [['gridColumn', num(`span ${m[1]} / span ${m[1]}`)]];
...
function sizeVal(raw) {
	...
	if (frac)
		return `${((Number(frac[1]) / Number(frac[2])) * 100).toFixed(6).replace(/\.?0+$/, '')}%`;
...
for (const file of files) {
	...
	for (const e of edits)
		src = src.slice(0, e.index) + e.replacement + src.slice(e.index + e.length);
...
for (const r of report) {
	console.log(`${r.file}`);
	console.log(
		`  converted: ${r.convertedInFile}${r.skipped ? `  | skipped (has style=, merge manually): ${r.skipped}` : ''}`,
	);
	if (r.flagged.length)
		console.log(`  FLAGGED (left in className, review): ${r.flagged.join(' ')}`);
}
console.log(
