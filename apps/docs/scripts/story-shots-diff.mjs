...
const highlight = hexToRgb(opts.tint || (opts.mode.startsWith('green') ? '#00e05a' : '#ff003a'));
...
const writeRgba = ({ width, height, data }, file) =>
	writeFile(file, magick(['-depth', '8', '-size', `${width}x${height}`, 'RGBA:-', 'png:-'], data));
...
const diffPair = (base, after, mode) => {
	...
	const extra =
		after.width * after.height - width * height + (base.width * base.height - width * height);
...
const [baseRun, afterRun] = await Promise.all([manifest(baseDir), manifest(afterDir)]);
...
const themeOf = (rel) =>
	(shotOf(afterRun, rel) ?? shotOf(baseRun, rel))?.theme ?? rel.split(path.sep)[0];
...
const sideLabel = (side, run) => [side, settings(run, changedKeys)].filter(Boolean).join('   ');
...
const montage = ({ tiles, width, background, foreground, caption, target, theme }) => {
	magick([
		'-background',
		background,
		'-fill',
...
const [baseFiles, afterFiles] = await Promise.all([pngs(baseDir), pngs(afterDir)]);
const inBase = new Set(baseFiles);
const inAfter = new Set(afterFiles);

for (const rel of [...new Set([...baseFiles, ...afterFiles])].sort()) {
...
await Promise.all(Object.values(scratch).map((file) => rm(file, { force: true })));

results
	.sort((a, b) => b[0] - a[0])
	.forEach(([changed, rel, note]) =>
		console.log(`${String(changed).padStart(10)}  ${rel}${note ? `  (${note})` : ''}`),
	);
