...
import { CONFIG_KEYS, hasMagick, settingsLine, stamp } from './story-shots-caption.mjs';

/** The wall clock every shot is taken at, so a story that reads the date is stable across runs. */
const FROZEN_CLOCK = '2026-06-15T12:00:00.000Z';

/** `.dark` on `<html>` is what the preview themes on, the class ModeDecorator toggles. */
...
const { values: opts, positionals } = parseArgs({
	...
	options: {
		...
		grow: { type: 'string', default: 'document' },
...
if (opts.help || (!outDir && !opts.list)) {
  ...
  --theme <themes>    themes to shoot: dark, light or dark,light (default: dark,
                      the preview's own default)
  ...
  --grow <what>       document (default) grows the viewport until the page fits,
                      none keeps --height and leaves the page its own scrollbar
...
Shots use playwright's own chromium; override the binary with CHROME_PATH.`);
...
const index = await fetch(`${base}/index.json`)
	...
	.catch(() => {
		console.error(`no storybook at ${base}. Start one with 'pnpm dev' or pass --port.`);
		return process.exit(1);
	});

const matches = opts.stories.flatMap((value) => value.split(',')).filter(Boolean);
...
const stories = Object.values(index.entries)
	.filter((entry) => {
		...
		if (opts.name && !entry.name.toLowerCase().includes(opts.name.toLowerCase())) {
			return false;
		}
		if (!matches.length) {
			return true;
...
if (unknownTheme) {
	console.error(`--theme: expected ${THEMES.join(' or ')}, got: ${unknownTheme}`);
	process.exit(1);
}
 ...
 * held still - the parked animations, the lists snapped onto their bottom - is
 * done by the preview itself, so a Chromatic build and a shot from here see the
 * same page.
 */
const ignoreCss = (ignore) => `[data-shot-ignore], [data-chromatic='ignore']${ignore
...
console.log(`${stories.length} stories x ${themes.length || 1} theme(s) -> ${outDir}`);
...
for (const theme of themes.length ? themes : [null]) {
	...
	for (const story of stories) {
		// A context per story: nothing a story writes to the document, to storage
		// or to the theme class carries into the next one.
		const context = await browser.newContext({
			viewport: { width: Number(opts.width), height: Number(opts.height) },
			reducedMotion: opts.motion ? 'no-preference' : 'reduce',
		});
		if (opts.clock !== 'live') {
			// Timers keep running; only what the page reads as the date is pinned.
			await context.clock.setFixedTime(new Date(opts.clock));
		}
		const page = await context.newPage();

		// Webfonts and lazily loaded story chunks keep requests going after load,
		// so the settle waits on the page being quiet rather than on a fixed delay.
		let inFlight = 0;
		...
		// The preview owns the motion state, so it is asked for in the URL rather
		// than injected here: a Chromatic build gets the default.
		if (opts.motion) {
			url.searchParams.set('globals', 'motion:live');
		}
		if (storyArgs) {
		...
		try {
			...
			// ModeDecorator sets the class on mount, so the theme is forced after the
			...
			while (Date.now() < quietUntil && (inFlight > 0 || Date.now() - lastActivity < 600)) {
				await page.waitForTimeout(200);
			}
			await page.waitForTimeout(Number(opts.settle));

			// The width is the fixed dimension and the height follows the page, the
			// way a Chromatic viewport does. Growing changes the layout, hence the
			// rounds. A story that sizes itself in `vh` grows its own content as the
			// viewport grows, so no height ever fits it and the rounds only chase:
			// such a story is shot at `--height` with its own scrollbar instead,
			...
			// page went quiet; a virtualized list is usually still measuring then.
			...
			// tooltip being repositioned - is shot twice in a row until two frames
			// come back identical, since what the page is waiting on is not
			...
			let caption = 0;
			if (captioning) {
				// Written beside the shot rather than in the system temp dir: the
				// rename onto it has to stay on one filesystem.
				const temporary = path.join(dir, `.${story.id}.caption.png`);
				caption = stamp({
					lines: [
			...
			shots.push({
				file: path.posix.join(opts.flat ? '' : (theme ?? 'default'), `${story.id}.png`),
				id: story.id,
				title: story.title,
				name: story.name,
				theme: theme ?? 'default',
