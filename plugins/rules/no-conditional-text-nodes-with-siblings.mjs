...
function isConditionallyRendered(node) {
	...
	return parent?.type === 'ConditionalExpression' || parent?.type === 'LogicalExpression';
}
...
function isPrecededByConditional(node) {
	...
	return children.some((child) => child.start < node.start && isRenderedConditional(child));
}
...
// `display: contents` via CSS modules. React owns the element so Translate's
// `<font>` swap is absorbed, while the box tree stays as it was and a flex or
// grid parent still sees one contiguous text run.
// Import: import { translateSafeClassName } from '@signozhq/ui/lib/translate-safe';
const OPEN = '<span className={translateSafeClassName}>';
const CLOSE = '</span>';
...
function wrapExpression(fixer, sourceCode, node) {
	...
	const target = node.parent?.type === 'JSXExpressionContainer' ? node.parent : node;
	...
	return fixer.replaceText(target, `${OPEN}{${sourceCode.getText(node)}}${CLOSE}`);
}
	...
	meta: {
		...
		messages: {
			'conditional-text-node':
				"Conditionally rendered text nodes with siblings, rendered as a direct child of a JSX element, must be wrapped so Google Translate cannot break React's DOM: `<span className={translateSafeClassName}>{value}</span>`. Import `translateSafeClassName` from `@signozhq/ui/lib/translate-safe`. Translate replaces the bare text node with a `<font>` element and React then throws on `removeChild`.",
			'text-node-preceded-by-conditional':
				"Text nodes which are preceded by a conditional expression, rendered as a direct child of a JSX element, must be wrapped so Google Translate cannot break React's DOM: `<span className={translateSafeClassName}>text</span>`. Import `translateSafeClassName` from `@signozhq/ui/lib/translate-safe`. Translate replaces the bare text node with a `<font>` element and React then throws on `removeChild`.",
		},
	},
