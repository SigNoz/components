	...
	meta: {
		...
		messages: {
			'return-value-is-text-node':
				'React components should avoid returning text nodes directly (or numerical values which will be rendered as text). Wrap with `<span className={translateSafeClassName}>{value}</span>`. Import `translateSafeClassName` from `@signozhq/ui/lib/translate-safe`. Google Translate can cause stale values after state changes without any error being thrown.',
		},
	},
			...
			return [
				{
					...
					fix: (fixer) =>
						fixer.replaceText(
							node.argument,
							`<span className={translateSafeClassName}>{${context.sourceCode.getText(node.argument)}}</span>`,
						),
				},
			];
		};
